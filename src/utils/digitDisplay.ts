 ...
 */
export function displayDigit(digit: number, format: DigitDisplay = 'numeric'): string {
  if (digit < 1 || digit > 9) return String(digit);

  switch (format) {
    case 'kanji':
      return KANJI_DIGITS[digit - 1];
    case 'emojis':
      return EMOJI_DIGITS[digit - 1];
    default:
      return String(digit);
  }
}
